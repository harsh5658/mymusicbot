...
from MyMusicBotMusic import YouTube, app
from MyMusicBotMusic.core.call import MyMusicBot
from MyMusicBotMusic.misc import SUDOERS, db
from MyMusicBotMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from MyMusicBotMusic.utils.decorators.language import languageCB
from MyMusicBotMusic.utils.formatters import seconds_to_min
from MyMusicBotMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from MyMusicBotMusic.utils.stream.autoclear import auto_clean
from MyMusicBotMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await MyMusicBot.pause_stream(chat_id)
        ...
        await MyMusicBot.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await MyMusicBot.stop_stream(chat_id)
        ...
        await MyMusicBot.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await MyMusicBot.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await MyMusicBot.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await MyMusicBot.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await MyMusicBot.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
